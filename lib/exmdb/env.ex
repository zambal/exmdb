defmodule Exmdb.Env do
  ...
  defp build_env_opts(opts) do
    ...
    env_opts =
      if Keyword.get(opts, :read_only) do
        [:read_only | env_opts]
      else
        env_opts
      end
    ...
    env_opts =
      if Keyword.get(opts, :write_map) do
        [:write_map | env_opts]
      else
        env_opts
      end
    ...
    env_opts =
      if Keyword.get(opts, :meta_sync, true) do
        env_opts
      else
    ...
    env_opts =
      if sync = Keyword.get(opts, :sync, true) do
        if sync == :async do
          [:map_async | env_opts]
        else
          env_opts
    ...
    env_opts =
      if Keyword.get(opts, :read_ahead, true) do
        env_opts
      else
  ...
  defp build_db_opts(opts, create) do
    ...
    db_opts =
      if Keyword.get(opts, :reverse_key) do
        [:reverse_key | db_opts]
      else
        db_opts
      end
    ...
    db_opts =
      if Keyword.get(opts, :dup_sort) do
        [:dup_sort | db_opts]
      else
        db_opts
      end
